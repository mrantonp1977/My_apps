...
def edit():
    ...
    file_app_label = Label(editor, text="ΕΦΑΡΜΟΓΗ :", font=("arial black", 16), background="#F3F3C3",
                           foreground="#7E0B9E")
    file_app_label.grid(row=0, column=0, pady=(10, 0), padx=15, sticky=W)
    username_label = Label(editor, text="USERNAME :", font=("arial black", 16), background="#F3F3C3",
                           foreground="#7E0B9E")
    username_label.grid(row=1, column=0, sticky=W, pady=10, padx=15)
    password_label = Label(editor, text="PASSWORD :", font=("arial black", 16), background="#F3F3C3",
                           foreground="#7E0B9E")
...
def select_app():
    ...
    if selected_app:
        c.execute("SELECT file_app, username, password FROM passwords WHERE file_app=? GROUP BY file_app",
                  (selected_app,))
        app_info = c.fetchall()
        if app_info:
            ...
            for row_index, worker_data in enumerate(app_info):
                for i, label in enumerate(info_labels):
                    label_text = Label(app_window, text=label, font=("arial black", 12), width=20, relief="ridge", bd=3,
                                       background="#F1B52D")
                    label_text.grid(row=row_index * len(info_labels) + i, column=0, padx=20, sticky=W, pady=10)

                    if i == 1:
                        # Display the sum of prices
                        value_label = Label(app_window, text=worker_data[i], font=("arial black", 13),
                                            foreground="yellow", background="#0C847F")
                    else:
...
title_label = Label(root, text="          MY PASSWORDS          ", font=("IMPACT", 20), background="black",
                    relief="ridge", borderwidth=10, foreground="yellow")
title_label.grid(row=0, column=0, columnspan=2, pady=20, padx=25)

submit_btn = Button(root, text="Εισαγωγή Στοιχείων", command=submit, bd=4, font=("arial black", 11),
                    background="#06D784", activebackground="#06D784", activeforeground="#06D784")
submit_btn.grid(row=6, column=0, pady=20, padx=10, ipadx=30)

show_btn = Button(root, text="Εμφάνιση Στοιχείων", command=show, bd=4, font=("arial black", 11), background="#6ECDF5",
                  activebackground="#6ECDF5", activeforeground="#6ECDF5")
show_btn.grid(row=6, column=1, pady=10, padx=10, ipadx=30)

delete_btn = Button(root, text="Διαγραφή Στοιχείων", command=delete, bd=4, font=("arial black", 11),
                    background="#F67EA3", activebackground="#F67EA3", activeforeground="#F67EA3")
delete_btn.grid(row=7, column=0, pady=10, padx=10, ipadx=30)

edit_btn = Button(root, text="Επεξεργασία Στοιχείων", command=edit, bd=4, font=("arial black", 11),
                  background="#A481C9", activebackground="#A481C9", activeforeground="#A481C9")
edit_btn.grid(row=7, column=1, pady=10, padx=10, ipadx=20)

select_btn = Button(root, text="Επιλογή Εφαρμογής :", command=select_app, font=("arial black", 9), background="blue",
                    foreground="yellow")
