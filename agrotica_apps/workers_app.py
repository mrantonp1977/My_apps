...
if __name__ == "__main__":
    ...
    def edit():
        ...
        name_editor_label = Label(editor, text=" Όνομα : ", font=("arial black", 12), background="#DBF0D7",
                                  foreground="#110464")
        name_editor_label.grid(row=2, column=0, sticky=W, padx=20, pady=10)
        work_editor_label = Label(editor, text=" Εργασία : ", font=("arial black", 12), background="#DBF0D7",
                                  foreground="#110464")
        work_editor_label.grid(row=3, column=0, sticky=W, padx=20, pady=10)
        hours_editor_label = Label(editor, text=" Ώρες : ", font=("arial black", 12), background="#DBF0D7",
                                   foreground="#110464")
    ...
    def select_worker():
        ...
        if selected_worker:
            c.execute("SELECT name, SUM(price), SUM(hours), work  FROM workers WHERE name=? GROUP BY name",
                      (selected_worker,))
            ...
            if worker_info:
                ...
                for row_index, worker_data in enumerate(worker_info):
                    for i, label in enumerate(info_labels):
                        label_text = Label(worker_window, text=label, font=("arial black", 12), width=20,
                        ...
                        if i == 1:
                            ...
                            value_label = Label(worker_window, text=worker_data[i], font=("arial black", 13),
                                                foreground="#62057C", background="#DBF0D7")
                        else:
    ...
    def open_cost_worker():
        ...
        total_cost_btn = Button(total_w, text=" ΣΥΝΟΛΙΚΟ ΚΟΣΤΟΣ ΕΡΓΑΤΩΝ ", font=("arial black", 11),
                                command=total_cost_workers, bd=6,
    ...
    date_label = Label(root, text=" Ημερομηνία : ", font=("arial black", 12), background="#DBF0D7",
                       foreground="#110464")
    ...
    price_label = Label(root, text=" Ημερομίσθιο € : ", font=("arial black", 12), background="#DBF0D7",
                        foreground="#110464")
    price_label.grid(row=5, column=0, sticky=W, padx=20, pady=10)
    choose_id_label = Label(root, text=" Επιλογή ID : ", font=("arial black", 12), background="#DBF0D7",
                            foreground="#110464")
    ...
    submit_btn = Button(root, text="Εισαγωγή Στοιχείων Εργάτη", command=submit, font=("arial black", 11), bd=4,
                        background="#06D784", activebackground="#06D784", activeforeground="#06D784")
    submit_btn.grid(row=8, column=0, pady=(20, 10), padx=10, ipadx=30)

    show_btn = Button(root, text="Εμφάνιση Στοιχείων Εργάτη", command=show, font=("arial black", 11), bd=4,
                      background="#0499A0", activebackground="#0499A0", activeforeground="#0499A0")
    show_btn.grid(row=9, column=1, pady=10, padx=10, ipadx=38)

    delete_btn = Button(root, text="Διαγραφή Στοιχείων Εργάτη", command=delete, font=("arial black", 11), bd=4,
                        background="#F67EA3", activebackground="#F67EA3", activeforeground="#F67EA3")
    delete_btn.grid(row=9, column=0, pady=10, padx=10, ipadx=30)

    edit_btn = Button(root, text="Επεξεργασία Στοιχείων Εργάτη", command=edit, font=("arial black", 11), bd=4,
    ...
    select_btn = Button(root, text="  Επιλογή Εργάτη : ", command=select_worker, font=("arial black", 11),
                        background="#0FAFC3")
    select_btn.grid(row=7, column=0, sticky=W, padx=10, pady=10)

    total_workers_cost_btn = Button(root, text="  ΣΥΝΟΛΙΚΟ ΚΟΣΤΟΣ ΕΡΓΑΤΩΝ : ", command=open_cost_worker,
                                    font=("arial black", 12), bd=6, background="#0FAFC3")
    total_workers_cost_btn.grid(row=10, columnspan=2, padx=10, pady=10)

    name_label = Label(root, text="Created and Designed by : Papaioannou Antonios", font=("arial black", 11),
                       foreground="grey", background="#DBF0D7", borderwidth=1)
